...
export const Gigs = () => {
  ...
  const gigs = data.gigs.sort((a, b) => new Date(a.dateTime).getTime() - new Date(b.dateTime).getTime());
  ...
  if (data.gigs.length) {
    return (
      <section>
        <div id="gigs" className="content-section mr-2">
          <Fade cascade direction="left" triggerOnce>
            <h1 className="text-bj-blue dark:text-bj-blue-light text-center text-2xl md:text-6xl h-min font-fredericka">
              Gigs
            </h1>
          {pastGigs.length ? (
            <div className="flex flex-row justify-center mt-6">
              <Button text={showPastGigs ? 'hide_past_gigs' : 'show_past_gigs'} onClick={handleClick} />
            </div>
          ) : null}
          </Fade>
        </div>
        <div className="mx-8 p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-16 justify-items-center">
          ...
          <Fade cascade direction={'up'} triggerOnce duration={700} damping={0.5}>
            {showPastGigs &&
              pastGigs.map((gig, index) => {
                return <EventCard gig={gig} status="past" key={index} />;
              })}
            {futureGigs.length && <EventCard gig={futureGigs[0]} status="next" />}

            {futureGigs.length > 1 &&
              futureGigs.slice(1, futureGigs.length).map((gig, index) => {
        ...
        {!futureGigs.length && (
          <div>
            <Fade direction={'up'} triggerOnce duration={700} damping={2}>
              <p className="mt-3 mx-24 text-md md:text-3xl text-bj-blue-dark dark:text-bj-white">{t('no_gigs')}</p>
            </Fade>
          </div>
        )}
      </section>
  ...
  return <p className="text-bj-blue dark:text-bj-blue-light h-min font-fredericka m-2">No events available</p>;
};
